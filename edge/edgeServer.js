...
function handleEdgeObserve(data, value) {
    ...
    for(idx in nodes) {
        ...
        //value = '{"devId":'+node.devInfo.id+',"serviceName":"temperature","attrName":"on","value":"0"}';
...
function registerNode(name) {
    getDevInfoFromName(name, function(devInfo) {
        console.log('find callback:'+devInfo);
        if (devInfo == null){
            return;
        }
        ...
        lwm2mServer.read(devInfo.id, 
            ...
            console.log('verifyProf');
            if (verifyProf(prof)) {
                console.log('verifyProf passed');
